...
CREATE OR REPLACE TABLE `logistics-data-storage-staging.long_term_pricing.valid_exp_names_switchback_tests` AS 
SELECT DISTINCT
  entity_id,
  country_code,
  test_id,
...
CREATE OR REPLACE TABLE `logistics-data-storage-staging.long_term_pricing.ab_test_target_groups_switchback_tests` AS
WITH vendor_tg_vertical_mapping_with_dup AS (
  ...
  WHERE TRUE 
    AND test_name IN (SELECT DISTINCT test_name FROM `logistics-data-storage-staging.long_term_pricing.valid_exp_names_switchback_tests`)
),
...
CREATE OR REPLACE TABLE `logistics-data-storage-staging.long_term_pricing.ab_test_zone_ids_switchback_tests` AS
SELECT DISTINCT -- The DISTINCT command is important here
    entity_id,
    country_code,
    test_start_date,
...
CREATE OR REPLACE TABLE `logistics-data-storage-staging.long_term_pricing.ab_test_tgs_variants_and_schemes_switchback_tests` AS
SELECT DISTINCT
    entity_id,
    country_code,
    test_start_date,
...
CREATE OR REPLACE TABLE `logistics-data-storage-staging.long_term_pricing.ab_test_agg_tgs_variants_and_schemes_switchback_tests` AS
SELECT 
  entity_id,
  country_code,
  test_name,
...
FROM `logistics-data-storage-staging.long_term_pricing.ab_test_tgs_variants_and_schemes_switchback_tests`
GROUP BY 1,2,3,4;

-- Step 5: Extract the polygon shapes of the experiment's target zones
CREATE OR REPLACE TABLE `logistics-data-storage-staging.long_term_pricing.ab_test_geo_data_switchback_tests` AS
SELECT 
    p.entity_id,
    co.country_code,
...
INNER JOIN `logistics-data-storage-staging.long_term_pricing.ab_test_zone_ids_switchback_tests` tgt ON p.entity_id = tgt.entity_id AND co.country_code = tgt.country_code AND zo.id = tgt.zone_id 
WHERE TRUE 
...
CREATE OR REPLACE TABLE `logistics-data-storage-staging.long_term_pricing.ab_test_individual_orders_switchback_tests` AS
WITH test_start_and_end_dates AS ( -- Get the start and end dates per test
  SELECT DISTINCT
    ...
    test_name,
    test_id
  FROM `logistics-data-storage-staging.long_term_pricing.ab_test_zone_ids_switchback_tests`
),
...
LEFT JOIN `logistics-data-storage-staging.long_term_pricing.ab_test_geo_data_switchback_tests` zn 
  ON TRUE 
...
LEFT JOIN `logistics-data-storage-staging.long_term_pricing.ab_test_target_groups_switchback_tests` tg -- Tag the vendors with their target group association
  ON TRUE
...
LEFT JOIN `logistics-data-storage-staging.long_term_pricing.ab_test_agg_tgs_variants_and_schemes_switchback_tests` vs -- Get the list of target_group | variation | scheme_id combinations that are relevant to the experiment
  ON TRUE
...
WHERE TRUE
    ...
    AND CONCAT(a.entity_id, ' | ', a.country_code, ' | ', a.experiment_id) IN ( -- Filter for the right entity | experiment_id combos. 
      ...
      FROM `logistics-data-storage-staging.long_term_pricing.ab_test_target_groups_switchback_tests`
...
CREATE OR REPLACE TABLE `logistics-data-storage-staging.long_term_pricing.ab_test_individual_orders_augmented_switchback_tests` AS
SELECT
  a.*,
...
FROM `logistics-data-storage-staging.long_term_pricing.ab_test_individual_orders_switchback_tests` a
...
CREATE OR REPLACE TABLE `logistics-data-storage-staging.long_term_pricing.vendor_locations_switchback_tests` AS
WITH vendor_list AS (
  SELECT DISTINCT
    entity_id,
    test_id,
    vendor_id,
  FROM `logistics-data-storage-staging.long_term_pricing.ab_test_individual_orders_augmented_switchback_tests`
)
...
FROM `logistics-data-storage-staging.long_term_pricing.ab_test_individual_orders_augmented_switchback_tests` a
LEFT JOIN `logistics-data-storage-staging.long_term_pricing.vendor_locations_switchback_tests` b USING (entity_id, test_id, vendor_id);
...
CREATE OR REPLACE TABLE `logistics-data-storage-staging.long_term_pricing.ab_test_individual_orders_cleaned_staging_switchback_tests` AS
SELECT
    *
FROM `logistics-data-storage-staging.long_term_pricing.ab_test_individual_orders_augmented_switchback_tests`
WHERE TRUE
